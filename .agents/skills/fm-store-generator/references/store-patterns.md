...
| Store | 文件 | 用途 |
|-------|------|------|
| `useAppAuthStore` | `modules/app/auth.ts` | 登录/登出、token |
| `useAppSettingsStore` | `modules/app/settings.ts` | 主题、布局配置 |
...
- 跨 store 调用：直接在 setup 函数内调用其他 store，如 `const authStore = useAppAuthStore()`
