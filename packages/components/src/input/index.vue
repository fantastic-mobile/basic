...
<template>
  <InputGroup :class="cn('w-[200px]', props.class)" @mouseenter="isHovered = true" @mouseleave="isHovered = false">
    ...
    <InputGroupAddon v-if="slots.end || props.clearable || props.type === 'password'" :align="props.align === 'inline' ? 'inline-end' : 'block-end'" :class="cn('empty:hidden', props.endClass)">
      <FmButton v-if="!props.disabled && props.clearable && !!value && (isFocused || isHovered)" variant="ghost" size="icon" type="button" class="size-6" @click="handleClear">
        <FmIcon name="i-lucide:x" />
      </FmButton>
      <FmButton v-if="props.type === 'password' && !!value" variant="ghost" size="icon" type="button" class="size-6" @click="type = type === 'password' ? 'text' : 'password'">
        <FmIcon :name="type === 'password' ? 'i-lucide:eye-off' : 'i-lucide:eye'" />
      </FmButton>
      <slot name="end" />
    </InputGroupAddon>
  </InputGroup>
